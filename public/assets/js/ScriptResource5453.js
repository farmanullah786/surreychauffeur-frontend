...
$type._resolveUrl = function WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('index-2.html') + 1);

    if (!url || !url.length) {
        return baseUrl;
    }

    if (url.charAt(0) === 'index-2.html') {
        var slashslash = baseUrl.indexOf('://');
        if (slashslash === -1) {
            throw Error.argument("baseUrl", Sys.Res.badBaseUrl1);
        }

        var nextSlash = baseUrl.indexOf('index-2.html', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('index-2.html');
        if (lastSlash === -1) {
